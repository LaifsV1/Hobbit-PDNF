...
(* up-to *)
let maxremembered = ref 100000
let upto_techs = ref "ngsrialfzuh"
...
let parse_debug str =
  ...
  let e = contains 'e' in
  let p = contains 'p' in
  (y,t,c,b,m,n,g,s,r,i,a,l,f,z,u,h,e,p)
...
let main =
  ...
  let speclist = [("-d", Arg.Set_string debug,
                   (def_msg_s "debug mode: e.g. \"ytcemngsrialfzuhp\" for t[y]pe-checking [t]races [c]onfigurations sym[b]olic-environment [m]emoisation [n]ormalisation [g]arbage-collection up-to-[s]eparation up-to-name-[r]euse up-to-[i]dentity sigma-g[a]rbage-collection sigma-norma[l]isation sigma-simpli[f]ication generali[z]ation remove-gamma-d[u]plicates reachable-beta-grap[h] l[e]af-configurations beta-gra[p]h-size" !debug));
                  ...
                  ("-u", Arg.Set_string (upto_techs),
                   (def_msg_s "up-to techniques: e.g. \"ngsrialfzuh\" for [n]ormalisation [g]arbage-collection up-to-[s]eparation up-to-name-[r]euse up-to-[i]dentity sigma-g[a]rbage-collection sigma-norma[l]isation sigma-simpli[f]ication generali[z]ation remove-gamma-d[u]plicates reachable-beta-grap[h]" !upto_techs))
                 ] in
