  ...
  let ret_confs =
    ...
    | ValExp (v, _) ->
       begin
         ...
         | NewRefECxt (({locid; str} as l), lt, e1, p) :: ec_rest  ->
            let newl  = if str = "_" then l else {locid = get_fresh_id (); str = str} in
            let extend str iold inew env =
              if str = "_" then env else (Substitution.IntMap.add iold inew env)
            in
            let newmap = extend l.str l.locid newl.locid Substitution.IntMap.empty in
            let newe1 = Substitution.loc_subst_exp newmap e1 in
            Some ({s = (store_update s0 newl v);
                   e = {ecxt = ec_rest; e = newe1}}, bound0, sigma0,list_subs) :: []
