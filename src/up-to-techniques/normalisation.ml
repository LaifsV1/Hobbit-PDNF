...
let rec normalise_exp : Ast.expression -> conf_names -> name_map -> Ast.expression =
  ...
  | IdentExp  (id, _) -> exp (* IdentExp (id_get id, None) *)
  ...
  | LetExp    (i, it, e1, e2, _) ->
  (* LetExp (id_get i, it, helper e1, helper e2, None) *)
     LetExp (i, it, helper e1, helper e2, None)
  | LetTuple  (is_ts, e1, e2, _) ->
  (* LetTuple (List.map (fun(a,b)->(id_get a,b)) is_ts, helper e1, helper e2, None) *)
     LetTuple (is_ts, helper e1, helper e2, None)
  ...
  (* MatchExp (t,helper e1,helper e2,id_get i1,id_get i2,helper e3,None) *)
     MatchExp (t,helper e1,helper e2, i1, i2,helper e3,None)
  and
normalise_val : Ast.value -> conf_names -> name_map -> Ast.value =
  ...
  (* FunVal (id_get fid, ft, id_get param, pt, normalise_exp e names sigma_names, None) *)
     FunVal (fid, ft, param, pt, normalise_exp e names sigma_names, None)
  | AbsCon (i, t, s, m) -> sigma_get {idid=i;str=s} (fun {idid;str} -> Ast.AbsCon (idid, t, str, m))
  | AbsFun (i, t1, t2, s, _) ->
     sigma_get {idid=i;str=s} (fun {idid;str} -> Ast.AbsFun (idid, t1, t2, str, None))
...
let normalise_cxt : Reductions_ast.eval_cxt -> conf_names -> name_map -> Reductions_ast.eval_cxt =
  ...
  let current_frame_locs : Reductions_ast.eval_frame -> Reductions_ast.eval_frame =
    ...
    begin
      ...
      | LetECxt (i, it, e2, _) ->
         (* LetECxt (id_get i, it, exp_normalise e2, None) *)
         LetECxt (i, it, exp_normalise e2, None)
      | LetTupleECxt (is_ts, e2, _) ->
         (* LetTupleECxt (List.map (fun(a,b)->id_get a,b) is_ts, e2, None) *)
         LetTupleECxt (is_ts, e2, None)
...
let mk_normalised_store : conf_names -> name_map -> Reductions_ast.store -> Reductions_ast.store =
  ...
  List.fold_right
    (fun (n,i) new_store ->
      ...
      let new_l =
        match check_ignore i old_l (Ast.{locid=i;str=loc_name}) with
        | Some x -> x
        | None -> assert false
      in
