...
module NameId = struct
  ...
  let compare {id=id1; str=str1}
              {id=id2; str=str2} = compare id1 id2
end
let string_of_name_id {id = i; str = s} = Printf.sprintf "%s_%d" s i
module NameMap = Map.Make(NameId)
type name_map = int NameMap.t
let namemap_add_fresh name_id nxt_id names_map =
  if NameMap.mem name_id names_map
  then false, names_map
  else true, NameMap.add name_id nxt_id names_map
...
type nxtid = { nxtvar : int ; nxtloc : int ; nxtabs : int }
let empty_nxtid = { nxtvar = 0 ; nxtloc = 0 ; nxtabs = 0 }
let inc_var nxtid = {nxtid with nxtvar = nxtid.nxtvar + 1}
let inc_loc nxtid = {nxtid with nxtloc = nxtid.nxtloc + 1}
let inc_abs nxtid = {nxtid with nxtabs = nxtid.nxtabs + 1}

type conf_names = { nxtid : nxtid ; vars : name_map ; locs : name_map ; abs : name_map }
let empty_conf_names = { nxtid = empty_nxtid ;
                         vars = NameMap.empty ; locs = NameMap.empty ; abs = NameMap.empty }
...
let id_ignore = -99
let check_ignore id og x = if id = id_ignore then Some og else (assert(id <> id_ignore);Some x)
let set_var_loc_ignore x = id_ignore

let get_mapped_id : Ast.ident -> conf_names -> Ast.ident option =
  fun ({idid;str} as input) names -> 
  match NameMap.find_opt {id=idid;str=str} names.vars with
  | None -> None
  | Some id -> check_ignore id input {idid=id;str=var_name}
let get_mapped_idloc : Ast.loc -> conf_names -> Ast.loc option =
  fun ({locid;str} as input) names -> 
  match NameMap.find_opt {id=locid;str=str} names.vars with
  | None -> None
  | Some id -> check_ignore id input {locid=id+names.nxtid.nxtloc;str=loc_name}
let get_mapped_loc : Ast.loc -> conf_names -> Ast.loc option =
  fun ({locid;str} as input) names -> 
  match NameMap.find_opt {id=locid;str=str} names.locs with
  | None -> None
  | Some id -> check_ignore id input {locid=id;str=loc_name}
let get_mapped_abs : Ast.ident -> conf_names -> Ast.ident option =
  fun ({idid;str} as input) names -> 
  match NameMap.find_opt {id=idid;str=str} names.abs with
  | None -> None
  | Some id -> check_ignore id input {idid=id;str=abs_name}
let get_mapped_sigma : Ast.ident -> name_map -> Ast.ident option =
  fun ({idid;str} as input) names -> 
  match NameMap.find_opt {id=idid;str=str} names with
  | None -> None
  | Some id -> check_ignore id input {idid=id;str=sig_name}

(*** ADD FUNCTIONS ***)
let add_loc : Ast.loc -> conf_names -> conf_names =
  fun {locid; str} names ->
  let nxtid = names.nxtid in
  let success, newlocs = namemap_add_fresh {id=locid;str} nxtid.nxtloc names.locs in
  let newnxtid = if success then inc_loc nxtid else nxtid in
  {names with nxtid = newnxtid ; locs = newlocs}

let add_var_loc : Ast.loc -> conf_names -> conf_names =
  fun {locid; str} names ->
  let add_var : Ast.ident -> conf_names -> conf_names =
    fun {idid; str} names ->
    if idid < 0 then (* DON'T INCREMENT *)
      let success, newvars = namemap_add_fresh {id=idid;str} idid names.vars in
      {names with vars = newvars}
    else
      let nxtid = names.nxtid in
      let success, newvars = namemap_add_fresh {id=idid;str} nxtid.nxtvar names.vars in
      let newnxtid = if success then inc_var nxtid else nxtid in
      {names with nxtid = newnxtid ; vars = newvars}
  in
  add_var {idid=locid;str} names

let add_abs : int -> string -> conf_names -> conf_names =
  fun id str names ->
  let nxtid = names.nxtid in
  let success, newabs = namemap_add_fresh {id=id;str} nxtid.nxtabs names.abs in
  let newnxtid = if success then inc_abs nxtid else nxtid in
  {names with nxtid = newnxtid ; abs = newabs}

let add_sigma : int -> string -> (int * name_map) -> (int * name_map) =
  fun id str (nxtid,names) ->
  let success , newnames = namemap_add_fresh {id=id;str} nxtid names in
  if success then nxtid+1 , newnames
  else nxtid , names
...
let rec names_of_exp : exp_conf -> conf_names -> LocSet.t -> conf_names =
  ...
  | DerefExp  (l, p) -> 
     ...
     then acc
       ...
       begin
         ...
         | None -> acc (** doing this to normalise EEBetas **)
  ...
  | MatchExp (t,e1,e2,i1,i2,e3,p) ->
     ...
     names_of_exp {e=e3;st=s} acc1 exclude
           ...
           begin
             ...
             | TupleECxt (vs, es, p) ->
              ...
              | MatchECxt (t,e2,i1,i2,e3,p) ->
                 ...
                 names_of_exp {e=e3;st=s}
                   (names_of_exp {e=e2;st=s} acc exclude) exclude
           end
         in
...
let conf_names_of_sigma sigma conf_names =
  let nxtid,sigma_names = names_of_sigma sigma (conf_names.nxtid.nxtabs , conf_names.abs) in
  {conf_names with nxtid = {conf_names.nxtid with nxtabs = nxtid} ; abs = sigma_names}
...
let string_of_conf_names { nxtid ; vars ; locs ; abs } =
  Printf.sprintf "Next ID: %s;\nVariables: %s;\nLocations: %s;\nAlphas: %s"
    (Printf.sprintf "{nxtvar = %d; nxtloc = %d; nxtabs = %d}" nxtid.nxtvar nxtid.nxtloc nxtid.nxtabs)
...
let phi_removal conf_ids ((sigma,dtree),sigma_minus,phi,vars) =
  (* ids_of_prop: need names of prop because now RHS could contain stuff from generalisation... *)
  let ids_of_prop p acc =
    IntSet.union acc (intset_of_namemap (snd (names_of_prop p (0, NameMap.empty))))
  in
  ...
  let phi_var_clauses,phi_rest,phi_vars,phi_ids =
    ...
    let aux (top_vars,phi_rest,lhs_vars,lhs_ids) prop = 
      match prop with
      | TopIntEq (id , p) ->
         (top_vars,prop::phi_rest,(sint_of_id id)::lhs_vars,ids_of_prop p (IntSet.add id lhs_ids))
      | TopBoolEq (id , p) ->
         (top_vars,prop::phi_rest,(sbool_of_id id)::lhs_vars,ids_of_prop p (IntSet.add id lhs_ids))
      | TopBoolVar iv ->
...
let sigma_subset_removal (sigma,dtree) names1 names2 =
  ...
  (* 1: pop first top-level var, try to build PI and sigma_minus *)
