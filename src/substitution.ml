 ...
 * EDIT: removing Barendregt convention by removing the need to refresh.
 *       implementing instead a substutition for [x -> v]fun y.M if x=/=y
 * *)
let rec subst_gprop map g =
  ...
  | GIdent ({idid = id; str = s}, p) ->
     ...
     begin
       ...
       | None -> g
       | Some v -> g_of_v v (** REMOVED REFRESH **)
     end
...
let rec subst_val map v =
  ...
  | FunVal (fid, ft, param, pt, e, gen) ->
     (* added guard to avoid needing to refresh *)
     let map = (IntMap.remove fid.idid map) in 
     let map = (IntMap.remove param.idid map) in
     let newgen =
       match gen with
       | None -> None
       | Some (is, loc_exp_list, gprop) ->
...
and subst_exp map e =
  ...
  | LetExp   (id, it, e1, e2, p) ->
     (* added guard to avoid needing to refresh *)
     let map = (IntMap.remove id.idid map) in
     LetExp (id, it, (subst_exp map e1), (subst_exp map e2), p)
  | LetTuple (ids_ts, e1, e2, p)->
     (* remove from mapping all things already bound (i.e. no refreshing) *)
     let map = List.fold_left (fun newmap ({idid}, _) -> (IntMap.remove idid newmap)) map ids_ts in
  ...
  | MatchExp (t,e1,e2,i1,i2,e3,p) ->
     (* remove mapping to avoid capture without refreshing *)
     let map = (IntMap.remove i1.idid map) in
     let map = (IntMap.remove i2.idid map) in
...
(*****************************)
(* SUBST LOC TO REFRESH LOCS *)
(*****************************)
let rec loc_subst_val map v =
  match v with
  | ConstVal _ -> v
  | TupleVal vs ->
      TupleVal (List.map (fun v -> loc_subst_val map v) vs)
  | FunVal (fid, ft, param, pt, e, gen) ->
     let newgen =
       match gen with
       | None -> None
       | Some (is, loc_exp_list, gprop) ->
          let new_ls =
            List.map
              (fun ((l,p),e) ->
                match IntMap.find_opt (l.locid) map with
                | None -> (l,p), loc_subst_exp map e
                | Some i -> ({locid=i; str=l.str},p), loc_subst_exp map e)
              loc_exp_list
          in
          Some (is, new_ls, gprop) (** TODO: LOCS IN GPROP **)
     in
     FunVal (fid, ft, param, pt, loc_subst_exp map e, newgen)
  | AbsCon _ -> v
  | AbsFun (id,t1,t2,s,gen) ->
     let newgen =
       match gen with
     ...
     AbsFun (id,t1,t2,s,newgen)
  | ListVal (ls,t) ->
     ListVal (SymList.map (fun v -> loc_subst_val map v) (fun x -> x) ls, t)

and loc_subst_exp map e =
  match e with
  | ValExp   (v, p)          -> ValExp(loc_subst_val map v, p)         
  | IdentExp ({idid = id; str = _}, p) -> e
  | ArithExp (op, eLst, p)   -> ArithExp(op, (List.map (fun e -> loc_subst_exp map e) eLst), p)  
  | AppExp   (e1, e2, p)     -> AppExp ((loc_subst_exp map e1),(loc_subst_exp map e2), p)  
  | CondExp  (e1, e2, e3, p) -> CondExp((loc_subst_exp map e1),
                                        (loc_subst_exp map e2),
                                        (loc_subst_exp map e3), p)
  | NewRefExp (l, lt, e1, e2, p) -> let map = (IntMap.remove l.locid map) in (* remove locid *)
                                    NewRefExp (l, lt,
                                               (loc_subst_exp map e1),
                                               (loc_subst_exp map e2), p)
  | DerefExp (l, p) -> (match IntMap.find_opt (l.locid) map with
                        | None -> e
                        | Some i -> DerefExp ({locid=i; str=l.str}, p))
  | AssignExp (l, e, p) -> (let newe1 = loc_subst_exp map e in
                            match IntMap.find_opt (l.locid) map with
                            | None ->  AssignExp (l, newe1, p)
                            | Some i -> AssignExp ({locid=i; str=l.str}, newe1, p))
  | LetExp (id, it, e1, e2, p) -> LetExp (id, it,
                                          (loc_subst_exp map e1),
                                          (loc_subst_exp map e2), p)
  | LetTuple (ids_ts, e1, e2, p)-> LetTuple(ids_ts,
                                            (loc_subst_exp map e1),
                                            (loc_subst_exp map e2), p)
  | SeqExp (e1, e2, p) -> SeqExp    ((loc_subst_exp map e1),
                                     (loc_subst_exp map e2), p)    
  | TupleExp (es, p) -> TupleExp  (List.map (fun e -> loc_subst_exp map e) es, p)    
  | BotExp p -> e
  | TupleProjExp (e1, i, j, p) -> TupleProjExp (loc_subst_exp map e1, i, j, p)
  | TupleUpdExp (e1, i, j, e2, p) -> TupleUpdExp
                                       (loc_subst_exp map e1, i, j,
                                        loc_subst_exp map e2, p)
  | MatchExp (t,e1,e2,i1,i2,e3,p) -> MatchExp (t,
                                               (loc_subst_exp map e1),
                                               (loc_subst_exp map e2),i1,i2,
                                               (loc_subst_exp map e3),p)

let refresh_loc_exp ({locid; str} as l) exp =
  let newl  = if str = "_" then l else {locid = get_fresh_id (); str = str} in
  let extend str iold inew env = if str = "_" then env else (IntMap.add iold inew env) in
  let newmap = extend l.str l.locid newl.locid IntMap.empty in
  loc_subst_exp newmap exp
